...
export class CoreAPI {
    ...
    scan(options){
        ...
        }else if(existsSync(options.document_primary)){
            payload['file_base64'] = readFileSync(options.document_primary, {encoding: 'base64'});
        ...
        if(options.document_secondary){
            ...
            }else if(existsSync(options.document_secondary)) {
                payload['file_back_base64'] = readFileSync(options.document_secondary, {encoding: 'base64'});
        ...
        if(options.biometric_photo){
            ...
            }else if(existsSync(options.biometric_photo)){
                payload['face_base64'] = readFileSync(options.biometric_photo, {encoding: 'base64'});
        ...
        if(options.biometric_video){
            ...
            }else if(existsSync(options.biometric_video)){
                payload['video_base64'] = readFileSync(options.biometric_video, {encoding: 'base64'});
        ...
        return new Promise(function (resolve, reject) {
            post(apiURL, payload, {timeout: 60000, maxContentLength: Infinity, maxBodyLength: Infinity, headers: { 'content-type': 'application/json' }}).then(function (response) {
...
class DocuPass {
    ...
    createSignature(templateId, format = "PDF", prefillData = {}){
        ...
        return new Promise(function (resolve, reject) {
            post(apiURL, payload, {timeout: 15000, maxContentLength: Infinity, maxBodyLength: Infinity, headers: { 'content-type': 'application/json' }}).then(function (response) {
                resolve(response.data);
            }).catch(function (error) {
                if (error.response) {
                    reject(new Error('Connecting to API Server failed: HTTP '+  response.status));
...
class Vault {
    ...
    addImage(id, image, type = 0)
    {
        ...
        if(isValidURL(image)){
            ...
            payload['image'] = readFileSync(image, {encoding: 'base64'});
...
const _default = { CoreAPI, DocuPass, Vault, AMLAPI };
